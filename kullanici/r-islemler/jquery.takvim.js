...
(function($){
	...
	$.fn.mavitmTakvim = function(tConf){
		...
		function formatBelirle(yil,ay,gun){
			var stringText = tConf.tarihFormat;

			return stringText.replace('YYYY',yil).replace('Y',yil).replace('y',yil).replace('MM',ay).replace('M',ay).replace('m',ay).replace('DD',gun).replace('D',gun).replace('d',gun);
		}
		...
		function inputmavitmGunYaz(eleman, formInput){
			...
			var dateString = formatBelirle(clickYil, clickAy, clickGun);
		...
		function mavitmempty(mixed_var) 
		{var undef, key, i, len; var emptyValues = [undef, null, false, 0, '', '0'];for (i = 0, len = emptyValues.length; i < len; i++) {if (mixed_var === emptyValues[i]) {return true;}} return false;}
		function mavitmInArray(needle, haystack, argStrict)
